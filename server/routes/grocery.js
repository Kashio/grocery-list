...
protobuf.load(path.join(__dirname, '../../protos/grocery/api/response/grocery-delete.proto'))
    .then(root => {
...
router.delete('/', (req, res) => {
    Grocery.deleteGrocery(req.grocery)
        .then(result => {
            const message = GroceryDeleteResponseMessage.create({status: 1, grocery: req.grocery});
            res.setHeader("Content-Type", "application/octet-stream");
            res.write(GroceryDeleteResponseMessage.encode(message).finish());
            res.end();
        })
        .catch(error => {
            const message = GroceryDeleteResponseMessage.create({status: 2, message: error.message});
            res.setHeader("Content-Type", "application/octet-stream");
            res.write(GroceryDeleteResponseMessage.encode(message).finish());
            res.end();
        });
